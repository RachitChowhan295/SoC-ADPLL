...
module gain_scheduler #(
 ...
 // ---- Phase Thresholds ----
    ...
    // ---- Gain sets ----
    // FLL GAINS: Max integral gain (Shift 0 = /1 instead of /16)
    parameter [SHIFT_W-1:0] KP_SHIFT_FLL   = 5'd31, 
    parameter [SHIFT_W-1:0] KI_SHIFT_FLL   = 5'd0,  // Was 4 (16x more aggressive)

    // LARGE GAINS: Very aggressive phase pull-in
    parameter [SHIFT_W-1:0] KP_SHIFT_LARGE = 5'd0,  // Was 1 (2x more aggressive)
    parameter [SHIFT_W-1:0] KI_SHIFT_LARGE = 5'd3,  // Was 7 (16x more aggressive)

    // MEDIUM GAINS: Strong braking
    parameter [SHIFT_W-1:0] KP_SHIFT_MED   = 5'd1,  // Was 3 (4x more aggressive)
    parameter [SHIFT_W-1:0] KI_SHIFT_MED   = 5'd8,  // Was 13 (32x more aggressive)

    // FINE GAINS: Hold the center frequency
    parameter [SHIFT_W-1:0] KP_SHIFT_FINE  = 5'd3,  // Was 4 (2x more aggressive)
    parameter [SHIFT_W-1:0] KI_SHIFT_FINE  = 5'd12, // Was 16 (16x more aggressive)

    parameter [ERR_W-1:0] HYST = 32'd50
)(
    input  wire clk,
    input  wire rst,
    input  wire enable, // <--- NEW: Synchronizes with CIC Decimator
    ...
    // 1. Calculate Frequency Error ONLY when a new decimated phase sample arrives
    reg signed [ERR_W-1:0] phase_error_z1;
    always @(posedge clk or posedge rst) begin
        if (rst) phase_error_z1 <= 0;
        else if (enable) phase_error_z1 <= phase_error; // <--- FIXED
    end
    ...
    // State machine updates ONLY on enable strobe
    always @(posedge clk or posedge rst) begin
        if (rst) state <= FINE;
        else if (enable) state <= state_next; // <--- FIXED
    end
    ...
    always @(posedge clk or posedge rst) begin
        ...
        end else if (enable) begin // <--- FIXED
            case (state_next)
