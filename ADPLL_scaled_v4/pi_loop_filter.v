...
module pi_loop_filter #(
    parameter ERR_W      = 25,  // Matched to scheduler
    parameter SHIFT_W    = 5,   // Width of the shift inputs
    ...
    parameter signed [ACCUM_W-1:0] INTEG_MAX = 32'sd32767,
    parameter signed [ACCUM_W-1:0] INTEG_MIN = -32'sd32768
)(
    input  wire clk,
    input  wire rst,       
    input  wire enable,
    ...
    input wire signed [ACCUM_W-1:0] preload,
    ...
    // Compute terms using arithmetic right shift (DSP-free)
    wire signed [ACCUM_W-1:0]           p_term;
    wire signed [ACCUM_W+FRAC_BITS-1:0] i_term;
    
    assign p_term = $signed(error_ext[ACCUM_W-1:0]) >>> kp_shift;
    
    // Scale error up by FRAC_BITS for integrator precision, then apply shift attenuation
    assign i_term = $signed(error_ext <<< FRAC_BITS) >>> ki_shift;
    ...
    always @(posedge clk or posedge rst) begin 
        if (rst) begin                         
            integrator  <= $signed({preload, {FRAC_BITS{1'b0}}});
            ctrl_word_q <= preload;
            ctrl_word   <= preload[OUT_W-1:0];
        end
        else if (enable) begin
        // Only integrate if we're not already saturated in the same direction
