...
module adpll_top(
    input wire ref_clk,
    input wire board_clk, // <--- Fast clock for NCO
    input wire rst,
    ...
    // ====================================================
    // NEW: Mathematical "Digital TDC"
    // Bypasses the physical quantization trap completely!
    // ====================================================
    wire [6:0] dco_frac_gray; // Carries the perfect fractional phase from NCO
    ...
    digital_tdc tdc_inst (
        .clk_ref(ref_clk),
        .rst(rst),
        .dco_frac_gray(dco_frac_gray), // Pure math input from NCO
        .tdc_fine_out(fine_error)      // Output fractional error
    );
    ...
    dco_nco #(
        ...
        .dco_clk(dco_clk),
        .dco_frac_gray(dco_frac_gray) // <--- Exporting the fractional phase here
    );
